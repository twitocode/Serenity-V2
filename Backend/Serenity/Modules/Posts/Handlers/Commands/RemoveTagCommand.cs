...
public record RemoveTagCommand(ClaimsPrincipal Claims, string Id, string Tag) : IRequest<Response<object>>;

public class RemoveTagCommandHandler : IRequestHandler<RemoveTagCommand, Response<object>>
{
    private readonly DataContext context;
    private readonly UserManager<User> userManager;
    private readonly IMapper mapper;

    public RemoveTagCommandHandler(IMapper mapper, DataContext context, UserManager<User> userManager)
    {
        this.mapper = mapper;
    ...
    public async Task<Response<object>> Handle(RemoveTagCommand command, CancellationToken token)
    {
        ...
        if (!updatedPost.Tags.Contains(command.Tag))
        {
            return new()
            {
                Errors = new() { new("TagNotFound", $"The post with the Id of {command.Id} does not have the tag of {command.Tag}") },
                Data = null,
                Success = false
            };
        }

        else updatedPost.Tags.Remove(command.Tag);
